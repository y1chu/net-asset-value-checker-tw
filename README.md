...
- **首頁排行**：兩個排行榜 —「常用」（你的最愛）與「全部」（精選 40 檔熱門台股基金），
  依今日估算漲跌排序，可切換**漲幅 / 跌幅**。上方顯示**加權指數 / 櫃買指數**即時行情。
...
> 啟動時會在背景建立兩個索引並快取到 `data/`：全站約 4,400 檔基金的**名稱↔代碼**
> （搜尋用）、以及約 5,600 檔基金的**名稱↔鉅亨代碼**（淨值走勢用），所以查詢很快。
...
| `GET /api/estimates?codes=A,B` | 多檔基金的輕量估算＋淨值（常用清單用） |
| `GET /api/ranking` | 「全部」排行（精選 40 檔，快取）＋大盤 |
...
server/
  ...
  ranking.js     「全部」排行計算（快取 90s）
  curated.js     精選 40 檔熱門台股基金（scripts/select-curated.mjs 產生）
  market.js      加權 / 櫃買指數
public/          手機版單頁前端 + PWA（index.html / app.js / styles.css /
                 tokens.css / manifest.webmanifest / sw.js / icon-*.png）
scripts/         build-index.mjs（預建索引）、select-curated.mjs（挑選精選清單）
data/            fund-index.json / cnyes-index.json（索引快取，已提交供部署）
