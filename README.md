...
- **估算 → 官方自動切換**：盤中官方淨值還沒出時，各處顯示**今日估算漲跌**（標記「估」）；
  ...
  估算與其誤差則收進基金頁內。
- **首頁**：上方顯示**加權指數 / 櫃買指數**即時行情；「常用」排行榜列出你加入最愛的
  基金，依今日漲跌（估算或官方）排序，可切換**漲幅 / 跌幅**；**下拉可重新整理**。
- **持股重疊 / 集中度**（2 檔以上常用基金）：把常用基金的前十大持股加總，算出你的
  **有效曝險**（假設等額持有），並標示每檔股票被幾檔基金持有。全部基金都持有的個股
  會以紅色標記 — 重疊愈高，分散效果愈低。
...
- **基金頁**：
  ...
  - **官方淨值** — 最新公告淨值與日期。
  - **今日估算走勢** — 每次更新（30 秒）記錄一次估算值，畫出當日估算的變化路徑，
    含 0% 基準線，可看出盤中是持續下探還是正在回升。紀錄存在你的裝置。
...
| `GET /api/overlap?codes=A,B` | 多檔基金的持股重疊與有效曝險 |
