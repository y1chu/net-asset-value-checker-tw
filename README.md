...
- **估算 → 官方自動切換**：盤中官方淨值還沒出時，各處顯示**今日估算漲跌**（標記「估」）；
  一旦當日官方淨值公告後，摘要（首頁排行、常用清單）改顯示**官方淨值漲跌**（標記「淨」），
  估算與其誤差則收進基金頁內。
- **首頁**：上方顯示**加權指數 / 櫃買指數**即時行情；下方「常用」排行榜列出你加入最愛的
  基金，依今日漲跌（估算或官方）排序，可切換**漲幅 / 跌幅**。
- **搜尋基金**：打名稱或代碼（例：「安聯台灣科技」或 `ACDD04`），即時帶出結果。
- **基金頁**：
  - **今日漲跌** — 官方淨值出爐前顯示估算，出爐後顯示官方淨值漲跌，並列出「今日估算」與
    兩者**誤差**。估算為依已揭露前十大持股加權即時股價計算。
  - **大盤對比** — 顯示大盤今日漲跌，以及本基金「相對大盤」多少。
  - **官方淨值** — 最新公告淨值與日期。
  - **淨值走勢圖** — 近 1 月 / 3 月 / 6 月 / 1 年的每日淨值走勢（純 SVG，無外部套件）。
  - **持股明細** — 前十大個股：權重、現價、漲跌、對估算的貢獻（含漲停 / 跌停處理）。
  - **基金資料** — 近 1 月～5 年報酬、晨星評等、風險等級、經理費、規模、幣別、成立日。
  - **估算準確度** — 累積「估算 vs 官方淨值」的平均誤差（存在你的裝置，用久了才會出現）。
- **常用清單（側邊抽屜）**：點 ★ 加入；抽屜列出各基金**淨值**（每次開啟時更新）與今日漲跌。
- **深 / 淺色**：跟隨系統，可手動切換。
